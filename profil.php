...
if (isset($_GET['logout'])) {
    ...
    setcookie('user_id',  '', $cookie_options_del);
    setcookie('username', '', $cookie_options_del);
...
if (localStorage.getItem('theme') === 'dark') { html.classList.add('dark'); darkToggle.checked = true; }
else { html.classList.remove('dark'); darkToggle.checked = false; }
...
function showToast(type, message) {
    notifToast.classList.remove('hidden','flex','bg-green-50','border-green-200','text-green-700','bg-red-50','border-red-200','text-red-700','bg-amber-50','border-amber-200','text-amber-700','bg-blue-50','border-blue-200','text-blue-700');
    const styles = {
        success: ['bg-green-50','border-green-200','text-green-700','fas fa-check-circle'],
        error:   ['bg-red-50','border-red-200','text-red-700','fas fa-times-circle'],
        warning: ['bg-amber-50','border-amber-200','text-amber-700','fas fa-exclamation-circle'],
        info:    ['bg-blue-50','border-blue-200','text-blue-700','fas fa-info-circle'],
    };
    const s = styles[type] || styles.info;
    notifToast.classList.add('flex', ...s.slice(0,3));
    notifToastIcon.className = s[3] + ' text-base flex-shrink-0';
    notifToastMsg.textContent = message;
    clearTimeout(notifToast._hideTimer);
    notifToast._hideTimer = setTimeout(() => { notifToast.classList.add('hidden'); notifToast.classList.remove('flex'); }, 5000);
}
function updateStatusLabel(aktif) {
    notifStatusTxt.textContent = aktif ? `Aktif · pengingat jam ${String(JAM_NOTIFIKASI).padStart(2,'0')}.${String(MENIT_NOTIFIKASI).padStart(2,'0')} setiap hari` : 'Nonaktif';
}
let notifTimer = null;
function jadwalkanNotifikasi() {
    clearTimeout(notifTimer);
    const sekarang = new Date(); const target = new Date();
    target.setHours(JAM_NOTIFIKASI, MENIT_NOTIFIKASI, 0, 0);
    if (target <= sekarang) target.setDate(target.getDate() + 1);
    notifTimer = setTimeout(() => { kirimNotifikasi(); setInterval(kirimNotifikasi, 86400000); }, target - sekarang);
}
function kirimNotifikasi() {
    if (Notification.permission !== 'granted' || localStorage.getItem('notif_aktif') !== 'true') return;
    const n = new Notification('💰 DompetKos — Pengingat Harian', { body: 'Jangan lupa catat keuanganmu hari ini!', icon: 'favicon.ico', tag: 'dompetkos-harian' });
    n.onclick = () => { window.focus(); window.location.href = 'pencatatan.php'; n.close(); };
}
async function aktifkanNotifikasi() {
    ...
    if (perm === 'granted') {
        localStorage.setItem('notif_aktif','true'); updateStatusLabel(true); jadwalkanNotifikasi();
        showToast('success',`Notifikasi aktif! Pengingat jam ${String(JAM_NOTIFIKASI).padStart(2,'0')}.${String(MENIT_NOTIFIKASI).padStart(2,'0')}.`);
        ...
        notifToggle.checked = false; localStorage.setItem('notif_aktif','false'); updateStatusLabel(false);
        showToast(perm === 'denied' ? 'warning' : 'info', perm === 'denied' ? 'Izin diblokir browser. Buka pengaturan browser untuk mengizinkan.' : 'Izin belum diberikan.');
    }
}
function nonaktifkanNotifikasi() { clearTimeout(notifTimer); localStorage.setItem('notif_aktif','false'); updateStatusLabel(false); showToast('info','Notifikasi harian dinonaktifkan.'); }
notifToggle.addEventListener('change', function() { this.checked ? aktifkanNotifikasi() : nonaktifkanNotifikasi(); });
(function init() {
    ...
    if (localStorage.getItem('notif_aktif') === 'true' && Notification.permission === 'granted') { notifToggle.checked = true; updateStatusLabel(true); jadwalkanNotifikasi(); }
