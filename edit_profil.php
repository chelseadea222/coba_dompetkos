...
if (!$query) die("Gagal mengambil data user: " . $conn->error);
...
if (isset($_POST['update_profil'])) {
    ...
    if (isset($_FILES['foto_profil']) && $_FILES['foto_profil']['error'] == 0) {
        ...
        } elseif (!in_array($ekstensi, $ekstensi_ok)) {
    ...
    if ($pindah_foto_sukses) {
        ...
        if ($cek_email && $cek_email->num_rows > 0) {
            ...
            if ($conn->query($sql)) {
                // Update cookie username supaya tetap sinkron
                setcookie("username", $username_baru, ["expires"=>time()+86400,"path"=>"/","secure"=>true,"httponly"=>false,"samesite"=>"None"]);
...
<script>
    ...
    function togglePassword() {
        ...
        if (input.type === 'password') { input.type = 'text'; icon.className = 'far fa-eye-slash text-sm'; }
        else { input.type = 'password'; icon.className = 'far fa-eye text-sm'; }
    }
